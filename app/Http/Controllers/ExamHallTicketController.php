...
class ExamHallTicketController extends Controller
{
    public function index(Request $request)
    {
        ...
        $attendanceSummaries = AttendanceRecord::query()
            ...
            ->groupBy('students.id', 'students.enrollment_no', 'students.roll_no', 'students.class_section_id', 'students.program_id', 'users.name')
            ->get()
            ->map(function ($row) {
    ...
    public function studentHallTicket()
    {
        ...
        $isEligible = $percentage >= 75 || $hasWaiver;

        $overallToAttend = max(0, (int) ceil(3 * $conducted - 4 * $present));
        $overallToSkip = max(0, (int) floor((4 * $present - 3 * $conducted) / 3));

        return view('student.hall_ticket.show', compact('student', 'percentage', 'isEligible', 'hasWaiver', 'overallToAttend', 'overallToSkip'));
    }
    ...
    public function downloadHallTicket()
    {
        ...
        if (!$isEligible && !$user->isAdmin() && !($user->isHod() && $user->facultyProfile?->department?->department_code === 'EXAM')) {
            abort(403, 'Your End-Semester Hall Ticket is locked due to low attendance.');
        }
